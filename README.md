...
The hub loads no game code. `levels` in the catalogue is async and imports a
game's content only when a child opens that game's level path, so the cost of
the menu does not grow as games are added to it. Before that change, opening
the menu parsed every level table in the app plus one whole game
implementation: 20 modules and 180 KB, for a list of names.
...
node tools/test-hub.mjs           # the hub loads no game code; one path opens at a time
...
had always sent `bird`. `test-hub` asserts that the menu loads no game code
and that opening one path fetches exactly one game's content.
